...
public:
    ...
    virtual const LazyArray<uint16_t>& GetEntityList() const = 0;
};
...
public:
    ...
    const LazyArray<uint16_t>& GetEntityList() const override
    {
        return v_denseToSparse_;
    }

private:
    LazyArray<uint16_t> v_sparseArray_{MAX_ENTITIES};
    LazyArray<uint16_t> v_denseToSparse_{MAX_ENTITIES};
    LazyArray<T> v_denseArray_{MAX_ENTITIES};
};